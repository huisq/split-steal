...
module overmind::split_or_steal {
    ...
    */
    public entry fun create_game(
        ...
        coin::transfer<AptosCoin>(account, signer::address_of(&res_signer), prize_pool_amount);
    ...
    */
    public entry fun submit_decision(
        ...
        check_if_player_does_not_have_a_decision_submitted(game, signer::address_of(player));
        ...
        // TODO: Set the player's PlayerData decision_hash and salt_hash fields to the values provided in the params
        {
            ...
            let player_one_address = game.player_one.player_address;
            if(signer::address_of(player) == player_one_address){
    ...
    */
    public entry fun reveal_decision(
        ...
        };
        {
            ...
            let game = simple_map::borrow_mut(&mut state.games, &game_id);
            let decision;
            if(signer::address_of(player) == game.player_one.player_address){
        ...
        let player_one_address = game.player_one.player_address;
        let player_two_address = game.player_two.player_address;
        // TODO: If both players submitted their decisions:
        if((player_one_decision != 0) && (player_two_decision != 0)){
            ...
            if((player_one_decision == player_two_decision) && (player_one_decision == 1)){
                coin::transfer<AptosCoin>(&res_signer, player_one_address, prize_pool_amount/2);
                coin::transfer<AptosCoin>(&res_signer, player_two_address, prize_pool_amount/2);
            };
            ...
            if(player_one_decision == 1 && player_two_decision == 2){
                coin::transfer<AptosCoin>(&res_signer, player_two_address, prize_pool_amount);                
            };
            if(player_one_decision == 2 && player_two_decision == 1){
                coin::transfer<AptosCoin>(&res_signer, player_one_address, prize_pool_amount);                
            };
            //      4) If both players decided to steal, send the game's `prize_pool_amount` of APT to the deployer
            //          of the contract
            if((player_one_decision == player_two_decision) && (player_one_decision == 2)){
                coin::transfer<AptosCoin>(&res_signer, @overmind, prize_pool_amount);
            };
    ...
    */
    public entry fun release_funds_after_expiration(_account: &signer, game_id: u128) acquires State {
        ...
        let prize_pool_amount = game.prize_pool_amount;
        if((game.player_one.decision == game.player_two.decision) && (game.player_one.decision == 0)){
            coin::transfer<AptosCoin>(&res_signer, @overmind, prize_pool_amount);
        };
        //      2) The first player if the second player did not releave their decision
        if((game.player_one.decision != 0 ) && (game.player_two.decision == 0)){
            coin::transfer<AptosCoin>(&res_signer, game.player_one.player_address, prize_pool_amount);
        };
        //      3) The second player if the first player did not releave their decision
        if((game.player_one.decision == 0 ) && (game.player_two.decision != 0)){
            coin::transfer<AptosCoin>(&res_signer, game.player_two.player_address, prize_pool_amount);
        };
    ...
    */
    inline fun make_decision(player_data: &mut PlayerData, salt: &String): u64 {
        ...
        let split_vector = bcs::to_bytes(&DECISION_SPLIT);
        vector::append(&mut split_vector, salt_bytes);
        let split_decision = hash::sha3_256(split_vector);

        // TODO: Create a SHA3_256 hash of a steal decision from a vector containing serialized DECISION_STEAL const
        //      and bytes of the salt
        let steal_vector = bcs::to_bytes(&DECISION_STEAL);
        vector::append(&mut steal_vector, salt_bytes);
        let _steal_decision = hash::sha3_256(steal_vector);
        ...
        if(*option::borrow(&player_data.decision_hash) == split_decision){
